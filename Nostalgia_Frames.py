...
class RetrogamingOverlayApp:
    ...
    def update_listbox(self):
        ...
        for file in sorted_files:
            self.file_treeview.insert('', 'end', text=file)

        # Reselect the previously selected item, if applicable
        if selected_filename:
            # Find the item with the matching filename and reselect it
            for item in self.file_treeview.get_children():
    ...
    def create_overlay(self):
        ...
        selected_item = selected_items[0]  # Get the first (and should be only) selected item
        filename = self.file_treeview.item(selected_item, 'text')
        image_path = os.path.join(os.getcwd(), filename)
        original_image = Image.open(image_path)

        # Fetch the manually entered width and height for the transparent square
        try:
        ...
        # Calculate the ratio between the original and desired sizes
        width_ratio = desired_width / original_transparent_width
        height_ratio = desired_height / original_transparent_height

        # The new dimensions the image should be resized to
        ...
        # Now, we need to crop the resized image back to the original dimensions,
        # making sure the transparent square is centered
        center_x, center_y = resized_image.width // 2, resized_image.height // 2
        left = max(center_x - original_image.width // 2, 0)
        top = max(center_y - original_image.height // 2, 0)
        right = min(center_x + original_image.width // 2, resized_image.width)
        bottom = min(center_y + original_image.height // 2, resized_image.height)
        ...
        cropped_image.save(os.path.join(os.getcwd(), new_image_filename))
